...
def mmr(query_embedding, doc_embeddings, k=20, lambda_param=0.9):
    ...
    return selected
def mmr_fast(query_embedding, doc_embeddings, k=20, lambda_param=0.9):
    """Vectorized MMR using NumPy.
    - Normalizes all embeddings once
    - Precomputes doc-doc similarity matrix (NxN)
    - Selects up to k items with diversity
    This is significantly faster than the Python-loop version for N<=200.
    """
    try:
        if not doc_embeddings:
            return []
        E = np.asarray(doc_embeddings, dtype=np.float32)
        # Handle shape issues gracefully
        if E.ndim != 2:
            E = np.vstack([np.asarray(v, dtype=np.float32) for v in doc_embeddings])
        ...
        if k <= 0:
            return []
        # Similarity to query (N,)
        sim_to_query = E_norm @ q_norm
        # Precompute doc-doc similarity (N x N)
        doc_sim = E_norm @ E_norm.T
        selected = []
        ...
        for _ in range(k):
            if not selected:
                ...
                # Max similarity to any selected doc for each candidate
                max_to_selected = doc_sim[:, selected].max(axis=1)
                mmr_scores = lambda_param * sim_to_query - (1.0 - lambda_param) * max_to_selected
                # Exclude already selected
                mmr_scores[selected_mask] = -1e9
        ...
        # Fallback to the original implementation on any error
        return mmr(query_embedding, doc_embeddings, k=k, lambda_param=lambda_param)
